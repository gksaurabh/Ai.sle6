...
def create_embeddings(text):
    ...
    #return np.array(model.embed_documents(text)[0])    #use only if using openAI embedding model

#create an embedding for our querries
def create_query_embedding(query):
    return model.encode(query)
    #return np.array(model.embed_query(query))  #use only if using openAI embedding model
...
def combine_chunked_embeddings(headersWeights, product):
    ...
    #chunk_embeddings = np.array(chunk_embeddings)    #use only if using openAI embedding model
